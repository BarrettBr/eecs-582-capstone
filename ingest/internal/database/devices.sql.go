...
const getTempSampleCount = `-- name: GetTempSampleCount :one
SELECT COUNT(*) AS count
FROM temp_samples
`

func (q *Queries) GetTempSampleCount(ctx context.Context) (int64, error) {
	row := q.db.QueryRowContext(ctx, getTempSampleCount)
	var count int64
	err := row.Scan(&count)
	return count, err
}
